...
- motion=true는 최대 {n_motion}개(0개이면 모든 장면 motion=false).
- 이미지 프롬프트 규칙:
  * 반드시 실제 존재하는 항만·해안 토목 구조물(방파제, 테트라포드 소파블록, 케이슨, 호안, 준설선 등)의 실사 다큐멘터리 사진/단면 구도로 작성.
  * 절대 판타지, 공상과학, 아파트나 도시 빌딩 같은 뜬금없는 건축물을 넣지 말 것.
  * 테트라포드를 다룰 때는 '거대한 4발 콘크리트 테트라포드 블록들이 파도를 막고 있는 실사 모습'을 정밀 묘사.
  * 클로즈업, 항공 뷰, 파도가 부딪히는 단면 등 다양한 카메라 앵글을 골고루 배치.
"""
