...
- [시각 연출 및 2.5D 카메라 모션 규칙]:
  * 모든 장면은 신비한 건축사전식 '3D 엔지니어링 일러스트'를 기반으로, 자체 시네마틱 모션 엔진(초광각 돌리 줌, 틸트업 크레인, 헬리캠 패닝)을 통해 100% 역동적인 카메라 무빙 영상으로 변환됩니다.
  * "motion": false로 설정하고, 모든 장면에 대해 시각적으로 명쾌한 "image_prompt"를 작성하세요.
